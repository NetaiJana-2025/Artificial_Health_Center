...
# Load datasets
sym_des = pd.read_csv("dataset/symptoms_dataset.csv")
...
# Symptoms dictionary mapping symptom names to indices
symptoms_dict = {
    'itching': 0,
    'skin_rash': 1,
...
# Diseases list mapping numeric keys to disease names
diseases_list = {
    15: 'Fungal infection',
    4: 'Allergy',
    16: 'GERD',
...
def normalize_symptom(symptom, threshold=0.7):
    """
    If the symptom is not exactly in the dictionary, use fuzzy matching to
    find the closest match with a similarity of at least `threshold`.
    """
    if symptom in symptoms_dict:
        return symptom
    matches = get_close_matches(symptom, symptoms_dict.keys(), n=1, cutoff=threshold)
